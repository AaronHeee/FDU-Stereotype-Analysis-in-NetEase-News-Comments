...
def LDA(docs, num_topics=5, num_words=5):
    """
    Implement LDA model
...
def label_detect(text_list, num_topics=5, num_words=5):
    # read the selected titles
    docs = []
    for text in text_list:
        docs.append(preprocess(text))
    label = LDA(docs, num_topics, num_words)
    return label

# how to use it
text_list = ['在线社交网络：在Facebook和Twitter个体关系网中发现的人类认知约束', '线下社交：网络之外，才是真正属于你的圈子']
print(label_detect(text_list), num_topics=5, num_words=5)
